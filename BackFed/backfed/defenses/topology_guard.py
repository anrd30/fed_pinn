...
try:
    ...
    print("Warning: ripser not available. TopologyGuard will return dummy values.")
...
class TopologyGuard:
    ...
    def compute_persistence(self, logits: np.ndarray) -> Dict[str, Any]:
        ...
        if not RIPSER_AVAILABLE:
            # Return dummy values when ripser is not available
            return {
                'dgms': [],
                'max_persistence': 0.0,
...
# ── Integration Wrapper ───────────────────────────────────────────────
# (BaseDefense wrapper removed for BackFed integration)
